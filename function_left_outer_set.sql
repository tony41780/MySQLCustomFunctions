...
DROP FUNCTION IF EXISTS `left_outer_set` $$
CREATE FUNCTION `left_outer_set`( prmStr  text, prmStr2  text,prmDelimiter varchar(25)) RETURNS text CHARSET latin1
    DETERMINISTIC
BEGIN
  ...
  Set tmpOUTERLEFT = replace(replace(replace(tmpOUTERLEFT,"]][[",","),"]]",""),"[[","");
  return cast(tmpOUTERLEFT as char);
END $$
